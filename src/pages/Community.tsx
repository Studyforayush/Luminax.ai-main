...
const Community = () => {
  ...
  useEffect(() => {
    // Set five fixed communities
    const fixed: Community[] = [
      {
        id: 'cs-study',
        name: 'Computer Science Study Group',
        description: 'Algorithms, data structures, systems, and coding interviews.',
        category: 'Technology',
        memberCount: 1247,
        isPrivate: false,
        image: '',
        color: 'hsl(var(--accent))',
        tags: ['#CS', '#Algorithms', '#DSA'],
      ...
      },
      {
        ...
        description: 'Study together for exams, share resources, and support each other.',
        category: 'Healthcare',
        memberCount: 892,
        isPrivate: false,
        image: '',
        color: 'hsl(var(--primary))',
        tags: ['#Medicine', '#Anatomy', '#USMLE'],
        createdAt: new Date().toISOString(),
        rules: ['Be respectful', 'Cite sources', 'No medical misinformation'],
        moderators: ['1']
      },
      {
        id: 'eng-masters',
        name: 'Engineering Masters',
        description: 'Advanced engineering topics and collaborative problem solving.',
        ...
        createdAt: new Date().toISOString(),
        rules: ['Be respectful', 'Show your work', 'Constructive feedback only'],
        moderators: ['1']
      },
      {
        id: 'art-history',
        name: 'Art History Enthusiasts',
        description: 'Discuss movements, critique pieces, and share museum notes.',
        category: 'Arts',
        memberCount: 423,
        isPrivate: false,
        image: '',
        color: 'hsl(var(--primary))',
        tags: ['#History', '#Modern', '#Critique'],
        createdAt: new Date().toISOString(),
        rules: ['Be respectful', 'Credit artists', 'No hate speech'],
        moderators: ['1']
      },
      {
        id: 'future-biz',
        name: 'Future Business Leaders',
        description: 'Strategy, finance, entrepreneurship, and leadership skills.',
        category: 'Business',
        memberCount: 531,
        isPrivate: false,
        image: '',
        color: 'hsl(var(--primary))',
        tags: ['#MBA', '#Finance', '#Startup'],
        createdAt: new Date().toISOString(),
        rules: ['Be respectful', 'No solicitation', 'Share actionable insights'],
        moderators: ['1']
      }
    ];

    setCommunities(fixed);
    setSelectedCommunity(fixed[0]);
    setIsLoading(false);
  }, []);
  ...
  return (
      ...
      <Card className="mb-4 border-2 border-green-200 bg-card">
        ...
        <CardContent className="p-0">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 p-4">
            {filteredCommunities.map((community) => (
              <div
                key={community.id}
                className={`p-3 rounded-xl border-2 cursor-pointer transition-transform duration-200 hover:translate-x-1 ${
                  selectedCommunity?.id === community.id 
                    ? 'border-green-400 bg-card shadow-md' 
                    : 'border-green-200 bg-card hover:border-green-300 hover:shadow-sm'
                }`}
                onClick={() => setSelectedCommunity(community)}
              >
                <div className="flex flex-col items-center text-center gap-2">
                  <div 
                    className="w-10 h-10 rounded-lg flex items-center justify-center text-white font-bold shadow-sm bg-primary"
                  >
                  ...
                  <div className="w-full">
                    ...
                    <Badge variant="secondary" className="text-xs mb-1 bg-card text-green-700 border-green-200">
                      {community.category}
                    </Badge>
              ...
              <CardHeader className="pb-3">
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <div 
                      className="w-12 h-12 rounded-xl flex items-center justify-center text-white font-bold text-lg shadow-md flex-shrink-0 bg-primary"
                    >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-col sm:flex-row sm:items-center gap-2 mb-1">
                        ...
                        {joinedCommunities.has(selectedCommunity.id) && (
                          <Badge className="bg-purple-100 text-purple-700 border-purple-200 w-fit" style={{backgroundColor: '#457B9D', color: 'white', borderColor: '#457B9D'}}>
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                        <Badge variant="secondary" className="text-xs w-fit bg-card text-green-700 border-green-200">
                    ...
                    {joinedCommunities.has(selectedCommunity.id) ? (
                      <>
                        <Button 
                          ...
                          className="border-red-200 text-red-600 hover:bg-card hover:border-red-300 w-full sm:w-auto"
                        >
                ...
                <CardContent className="pt-0">
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-2">
                    {Array.from(joinedCommunities).map(communityId => {
                      ...
                      return (
                        <div key={communityId} className="flex flex-col items-center gap-2 p-2 bg-card border border-green-200 rounded-lg hover:shadow-sm transition-shadow">
                          <div 
